...
export default function HostCity() {
  const highlights = [
    {
      ...
      line2: "Charm",
      icon: (
        <svg className="w-6 h-6 text-[#C6112F] shrink-0" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
  ...
  return (
    <section className="relative w-full bg-white dark:bg-[#070a10] border-b border-neutral-200/60 dark:border-neutral-800 transition-colors duration-300 overflow-hidden">
      {/* SVG ClipPath for smooth curve */}
      <svg className="absolute w-0 h-0 pointer-events-none" aria-hidden="true">
        <defs>
          <clipPath id="host-city-curve" clipPathUnits="objectBoundingBox">
            <path d="M 0,0 L 0.65,0 C 0.82,0 0.95,0.3 1,1 L 0,1 Z" />
          </clipPath>
        </defs>
      </svg>

      <div className="max-w-[1440px] mx-auto min-h-[380px] sm:min-h-[440px] flex flex-col lg:flex-row items-stretch">
        ...
        <div className="w-full lg:w-[54%] py-12 sm:py-16 px-6 sm:px-10 lg:px-14 flex flex-col justify-center">
          <div className="mb-3">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-neutral-900 dark:text-white tracking-tight leading-[1.08] my-5">
            Québec City <br />
            A World-Class Destination
          </h2>

          <p className="text-neutral-600 dark:text-neutral-300 text-sm sm:text-base leading-relaxed mb-8 max-w-xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 divide-y sm:divide-y-0 sm:divide-x divide-neutral-200 dark:divide-neutral-800 pt-2">
            {highlights.map((item, idx) => (
              <div key={idx} className={`flex items-center gap-3 pt-3 sm:pt-0 ${idx > 0 ? "sm:pl-4" : ""}`}>
                {item.icon}
                <div className="flex flex-col text-xs sm:text-sm font-extrabold text-neutral-900 dark:text-white leading-tight">
